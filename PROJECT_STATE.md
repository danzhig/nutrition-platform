...
**Last updated:** 2026-05-20 (session 29)
...
A public-facing nutrition web app built on **Next.js 16 + Supabase + Vercel**, source-controlled on **GitHub**. The database layer is fully complete (291 foods × 58 nutrient definitions; all 58 nutrients have food data). The app has four main features: an interactive data table (sortable/filterable, no cell coloring), a meal/day planner, a calendar food log tracker, and a Diet Evaluator tab.
...
nutrition-platform/
├── app/
│   ├── layout.tsx              ← Root layout; html/body/fonts/metadata; wraps children in <AuthProvider> (no desktop gate — moved to (desktop)/layout.tsx in Mobile Phase 0)
│   ├── (desktop)/               ← Route group (no URL segment); desktop-only tree, added Mobile Phase 0
│   │   ├── layout.tsx          ← "Open on Desktop" mobile gate (md:hidden) + hidden md:contents wrapper; gate links to /m
│   │   └── page.tsx            ← Home: fetches app data server-side via fetchAppData(), renders <AppShell>; revalidate = 300
│   ├── m/                       ← Mobile app route, added Mobile Phase 1
│   │   ├── layout.tsx          ← viewport/metadata exports (viewportFit: 'cover', PWA/apple-web-app tags, manifest link); imports mobile.css
│   │   ├── mobile.css          ← Mobile-only CSS (tap highlight, overscroll containment, focus rings); not loaded on desktop
│   │   └── page.tsx            ← Server component: fetchAppData(), renders <MobileShell>; revalidate = 300
│   └── globals.css
├── public/m/
│   └── manifest.json           ← PWA manifest (start_url=/m, standalone, slate-900 theme/bg)
├── components/mobile/
│   ├── MobileShell.tsx         ← Root client wrapper: activeTab state, body-scroll lock, Visual Viewport API tab-bar pinning, bottom tab bar
│   ├── MobileHeader.tsx        ← Top bar: title, DV profile chip (placeholder), streak pill (placeholder, hidden at 0)
│   ├── MobileAccountScreen.tsx ← Full-page login/signup (logged-out) or email + DV profile row + Sign Out (logged-in); reuses useAuth()
│   ├── MobileDVProfileSheet.tsx ← Bottom sheet: None + built-ins + saved profiles, device-default star, first-selection toast, Android back-button dismissal
│   ├── MobileNutritionScreen.tsx ← Orchestrator: selectedFood/selectedGrams/unit state + localStorage persistence
│   ├── MobileFoodSearch.tsx    ← Search input + 20-result list + category <select> (temporary, see Ph-7)
│   ├── MobileGramInput.tsx     ← Tap-to-edit gram chip
│   ├── MobileUnitToggle.tsx    ← 3-pill %DV/srv/100g segmented control
│   ├── MobileNutrientAccordion.tsx ← 6 grouped, collapsible nutrient sections
│   ├── MobileNutrientRow.tsx   ← Single nutrient row: value + %DV bar
│   ├── MobileNutrientInfoSheet.tsx ← Bottom sheet: body role/deficiency/excess + top foods
│   ├── MobileNutrientRanking.tsx ← Top-N foods ranked bar list, tap to jump
│   ├── MobileCalendarScreen.tsx ← Orchestrator: selectedDate/weekStart/entriesByDate, ensureRangeLoaded, streak calc
│   ├── MobileWeekStrip.tsx     ← 7-day Mon–Sun pill row with entry dots + prev/next nav
│   ├── MobileDayLog.tsx        ← Macro chips + grouped entry cards + FAB + visualization card
│   ├── MobileVisualizationCard.tsx ← Swipeable snap-scroll donut (innerOnly) + radar pair
│   ├── MobileAddSheet.tsx      ← Food/Meal/Plan bottom sheet: search+badges, templates+presets, saved plans, diet suggestions row
│   └── useSwipeToDismiss.ts    ← Shared drag-to-dismiss hook for the 3 bottom sheets
├── components/
...
│   ├── MacroDonutChart.tsx     ← Dual-ring PieChart: inner = 4 macro slices (Net Carbs/Fibre/Protein/Fat); outer = top-5 foods per macro; `innerOnly?` prop (default false) skips the outer ring for compact mobile use
...
│   ├── portionSizes.ts         ← Per-food serving sizes (all 291 foods, keyed by food_id) + S/M/L size variants ← CRITICAL
...
│   ├── foodLogAdapters.ts      ← logItemToMealItem() — shared by desktop CalendarDayPanel and mobile MobileDayLog/MobileVisualizationCard
│   └── mobileDateUtils.ts      ← toISODate/fromISODate/addDays/mondayOf — mobile calendar date helpers
├── types/
...
| **Meal Comparison food drill-down** | ✅ Live — food pill buttons per meal panel (one per food + "All"); clicking shows that food's standalone %DV contribution; diff panel always compares full meals (`MealComparisonView.tsx`); **bug fix:** GI is computed as a carb-weighted average (matching MealNutritionSidebar logic) in both `MealComparisonView` and `FoodComparisonView`; prior versions incorrectly scaled GI × grams, inflating reported %DV vs the Day Planner for the same food |
...
| **Cooked meat/poultry/fish variants** | ✅ Live — 34 new food entries (IDs 258–291), one cooked counterpart for every raw Red Meat/Poultry/Fish & Seafood food that lacked one (Sardines excluded, already a cooked product); values derived from each food's raw data via a cooking-loss model (`sql/insert_cooked_meat_fish.mjs`) — per-food cooked-yield % concentrates protein/minerals/water-stable nutrients, a separate fat-retention % accounts for rendered/drained fat (bacon, pork belly, ground beef, duck breast lose proportionally more fat than water), and heat-labile vitamins (C, B1, B2, B6, folate, pantothenic acid) get an added retention discount on top of concentration; water and calories recomputed for mass/Atwater consistency; `portionSizes.ts` cooked-serving grams reflect real shrinkage, not copied from the raw entry |
...
| **Diet Evaluator — per-food serving/grams toggle** | ✅ Live — each food row in Your Diet has a [srv \| g] mode toggle; **srv mode** (default) uses the food's standard portion size and displays the default grams as a muted hint (e.g. "172g"); **g mode** shows a compact number input pre-filled with the portion size so the user can enter any gram amount; `gramsOverride` field added to `DietFood` in `lib/dietStorage.ts`; when set, replaces the default portion size in `computeDietProfile`, `computeFoodContribs` in `DietNutrientPanel`, and the monthly composition bar segments; switching back to srv clears the override and restores the standard serving weight |
| **Diet tab — Suggestions panel moved to top** | ✅ Live — "Foods that would strengthen your diet" section now renders above the Browse Foods / Your Diet / Nutrient Coverage columns; users can add recommended foods without scrolling, and the suggestions update live as foods are added |
| **Diet Evaluator — per-food %DV distribution bar** | ✅ Live — clicking any nutrient row in Nutrient Coverage panel opens `NutrientInfoCard` with a stacked proportional bar at the top showing which selected foods contribute to that nutrient's %DV; "X% DV total" badge; any nutrient is clickable when foods are selected (not just those with `body_role`); `DietNutrientPanel` computes `computeFoodContribs()` at click time using normalized weights; `NutrientInfoCard` accepts optional `dietContribs` + `dietTotalPctDV` props; `DietView` passes `selectedFoods` + `dailyWeightG` to `DietNutrientPanel`; **bug fix (session 26):** `computeFoodContribs` now has a weighted-average branch for GI (mirrors `computeDietProfile` exactly — each food's contribPctDV = GI × weight-share / rdaTarget); "X% DV total" badge now uses `result.pctDV` from `computeDietProfile` as authoritative source rather than summing the truncated top-8 contribs, so the card total always matches the Nutrient Coverage bar; `WEIGHTED_AVERAGE_NUTRIENTS` exported from `dietProfile.ts`; when more than 8 foods contribute to a nutrient, remaining foods are aggregated into an "Other" catch-all segment (slate-600 color) appended after the named top-8 so the bar accounts for all contributors |
...
| **Default DV Profile** | ✅ Live — star (☆/★) toggle on every profile row in the DV Profile overlay picker (None, four built-ins, each saved profile); logged-in only; filled amber ★ = current account default; clicking a star saves to a sentinel row (`name = '__np_prefs__'`) in the existing `user_rda_profiles` table (no new table needed); clicking the active star deletes the row; on login the DB default overrides localStorage and is applied immediately; no default set = localStorage behaviour unchanged; `lib/userPreferencesStorage.ts` created (load / save / delete via sentinel pattern); `loadSavedProfiles` filters out the sentinel with `.neq('name', PREFS_SENTINEL)`; saved-profiles and prefs loads are independent (one failure can't block the other); new props `defaultProfileKey` + `onSetDefault` wired from `AppShell` → `DVProfilePanel`; built-in profile rows restructured from plain `<button>` to `<div>`+button+star in overlay picker |
| **Mobile — Phase 0: Route Unblock** | ✅ Complete — desktop page + "Open on Desktop" gate moved into `app/(desktop)/` route group; root layout reduced to html/body/fonts/`AuthProvider` so `/m` is no longer hidden by the `md:hidden` gate; mobile entry link (`Open the mobile app →`) added to the gate, pointing to `/m`. Desktop `/` unchanged (verified via `tsc --noEmit` clean + `next build` — build only fails on missing local Supabase env vars, a pre-existing condition unrelated to this change). `app/m/` itself is not created yet — that's Phase 1. |
| **Mobile — Phase 1: Foundation** | ✅ Complete — `/m` route + `app/m/layout.tsx` (`viewport`/`metadata` exports: `viewportFit: 'cover'`, PWA/apple-web-app tags, manifest link; no hand-written `<meta>` tags) + `app/m/page.tsx` (server component, `fetchAppData()`, `revalidate = 300`, renders `<MobileShell>`); `public/m/manifest.json` PWA manifest; `MobileShell.tsx` (owns `activeTab` state, body-scroll lock with unmount restore, Visual Viewport API effect pinning the tab bar above an open keyboard, bottom tab bar with Calendar/Nutrition/Account inline-SVG icons, `env(safe-area-inset-bottom)` padding, screen placeholders for Ph-2/4/5); `MobileHeader.tsx` (title + DV profile chip placeholder "No Profile" + streak pill placeholder, hidden at 0). Mobile CSS (tap highlight, overscroll containment, focus rings, touch-action) shipped as `app/m/mobile.css`, imported only by the mobile layout — desktop CSS untouched. No new icon library added (inline SVG). `tsc --noEmit` clean; `next build` compiles/typechecks clean but page-data collection for both `/` and `/m` fails in this dev environment due to missing `.env.local` Supabase credentials (pre-existing, unrelated to this phase). `/m` RSC transfer size **not yet measured** — blocked on the same missing credentials; flagged as a follow-up once Supabase env vars are available. |
| **Mobile — Phase 2: Account Screen** | ✅ Complete — `MobileAccountScreen.tsx`: logged-out full-page form (email+password, `text-base` 16px inputs, `autoComplete/autoCorrect/autoCapitalize` off, `spellCheck={false}`, inline red error / emerald info banners matching `AuthModal.tsx`'s pattern, Sign In ⇄ Sign Up toggle, no demo/tour links); logged-in state (email display, "Default DV Profile" row reading `np:m:rda-sel` from localStorage — placeholder until Ph-3 writes it, non-destructive slate-bordered Sign Out button). Reuses `useAuth()` from `AuthProvider` — identical session to desktop. Wired into `MobileShell`: replaces the Account tab placeholder; `onLoginSuccess` switches `activeTab` to `'calendar'`. |
| **Mobile — Phase 3: DV Profile Sheet** | ✅ Complete — `MobileDVProfileSheet.tsx`: bottom sheet (None + 4 built-ins + logged-in user's saved profiles under a divider), radio-style rows with a violet active dot, per-row star toggling a device-local default (`np:m:rda-default` / `np:m:rda-default-set`), 2.5s "Set as default on this device?" toast on first-ever selection, backdrop tap + Android back-button (`pushState`/`popstate` sentinel) both dismiss. `MobileShell` now owns `rdaSelection` (persisted to `np:m:rda-sel`), loads `savedProfiles` via `loadSavedProfiles()` when signed in, and resolves `rdaProfile` via `getProfile()` (built-ins) or an inline saved-profile resolve — same pattern as desktop `AppShell.tsx`. Also introduces a single hoisted `openSheet: 'dv' \| 'nutrient' \| null` state on `MobileShell` (Ph-4c/Ph-5b reuse it for the nutrient info sheet). `MobileHeader`'s DV chip is wired to open the sheet and shows the active profile's short label. `MobileAccountScreen`'s "Default DV Profile" row now reads the live resolved profile instead of a raw localStorage key. Custom-profile editing not built yet (built-ins + pre-made saved profiles only), per this phase's own scope note. |
| **Mobile — Phase 4a: Nutrition Core Controls** | ✅ Complete — `MobileNutritionScreen.tsx` orchestrator (owns `selectedFood`/`selectedGrams`/`unit`, persisted to `np:m:nutrition-food-id`/`-grams`/`-unit`; receives `foods` + `rdaProfile` from `MobileShell`); `MobileFoodSearch.tsx` (name-match search, up to 20 results, native `<select>` category filter — flagged for Ph-7's "No Native Selects" audit); `MobileGramInput.tsx` (tap-to-edit chip ↔ `inputMode="decimal"` input, `scrollIntoView` on focus, confirms on blur/Enter); `MobileUnitToggle.tsx` (3-pill `%DV`/`/srv`/`/100g` segmented control). Selecting a food collapses the search bar behind a food card with a "× change food" link; gram default comes from `getPortionSize()`. Wired into `MobileShell`'s Nutrition tab. |
| **Mobile — Phase 4b: Nutrient Accordion** | ✅ Complete — `MobileNutrientAccordion.tsx`: 6 groups from `NUTRIENT_GROUP_LIST` (membership derived from `nutrient.nutrient_category`, not a member-list), grid-rows CSS-transition expand/collapse, Macronutrients open by default, expansion state persisted to `np:m:nutrient-groups`; `MobileNutrientRow.tsx` (name + value + %DV bar via `rdaCellColor`, muted when no RDA target, bar shown only in `%DV` unit mode). Full `/100g` / `/srv` / `%DV` computation wired, including the `FOOD_METRIC_TARGETS` fallback for the Food Metrics group. Calories not special-cased (matches desktop, which doesn't special-case it either). Wired into `MobileNutritionScreen`. |
| **Mobile — Phase 4c: Nutrition Advanced Features** | ✅ Complete — `MobileNutrientInfoSheet.tsx` (bottom sheet: Function/Too Little/Too Much from `body_role`/`deficiency_symptoms`/`excess_symptoms`, "Top Foods" ranking, Android back-button dismissal); `MobileNutrientRanking.tsx` (top-N foods by per-serving or per-100g value, tap to jump); "Log to Today" sticky bar above the tab bar (`addEntry()` with the exact `NewFoodLogEntry` payload shape, 1.5s toast, sign-in guard that switches to the Account tab). Nutrient-sheet open state hoisted onto `MobileShell` (`openSheet: 'dv' \| 'nutrient' \| null` + `nutrientSheetName`) rather than owned locally by the Nutrition screen, per the Ph-3 note that Ph-5b's Calendar macro chips will need to open the same sheet. A small `jumpToFood` hand-off on `MobileShell` lets "Top Foods" rows switch the Nutrition screen to a different food from within the sheet. |
| **Mobile — Phase 5a: Calendar Week Strip + Day Log** | ✅ Complete — `MobileCalendarScreen.tsx` orchestrator (selected date persisted to `np:m:cal-date`, entries loaded via `getEntriesForDateRange` guarded on `userId`, `ensureRangeLoaded()` widens the fetch window when week navigation goes outside it); `MobileWeekStrip.tsx` (7-day Mon–Sun pill row, violet ring on selected day, violet fill on today, entry dot indicator, prev/next week nav); `MobileDayLog.tsx` (read-only entry cards grouped by `entry_type` with `meal_label` sub-grouping for plan entries — no breakfast/lunch/dinner concept, matches the data model — tap header to expand items, "No entries for this day" empty state, `+` FAB above the tab bar). New `lib/mobileDateUtils.ts` (`toISODate`/`fromISODate`/`addDays`/`mondayOf`). Desktop `CalendarView.tsx`/`CalendarDayPanel.tsx`/`CalendarWeekList.tsx`/`CalendarMonthGrid.tsx` untouched except the Ph-5b adapter extraction below. |
| **Mobile — Phase 5b: Calendar Summary + Visualizations** | ✅ Complete — 4-chip macro row (`🔥 kcal · P · F · C`) at the top of `MobileDayLog`, turns violet per-chip when that macro hits its DV target, tap opens the nutrient info sheet; streak pill in `MobileHeader` (🔥 N, computed from the same 30-day `entriesByDate` map Ph-5a already loads — no second fetch — counts the unbroken run ending yesterday, includes today once logged, hidden at 0); `MobileVisualizationCard.tsx` (swipeable `snap-x` donut + radar pair with a 2-dot indicator, shown only when the day has entries and a DV profile is active). `lib/foodLogAdapters.ts` created with `logItemToMealItem()` extracted from `CalendarDayPanel.tsx` (which now imports it — desktop behavior unchanged, verified live). `MacroDonutChart.tsx` gained an additive `innerOnly?: boolean` prop (skips the outer ring, tighter label radius) — desktop's default rendering is unaffected. GI excluded from the shared day-totals map (matches `MealNutritionSidebar`'s carb-weighted-average handling; GI needs a weighted average, not a sum). Verified against live Supabase data: `next build` clean, dev-server smoke test of both `/` and `/m` with no runtime errors. |
| **Mobile — Phase 6: Add Food/Meal Sheet** | ✅ Complete — `MobileAddSheet.tsx`: 80vh bottom sheet, Food/Meal/Plan tabs, Android back-button sentinel, open state owned by `MobileCalendarScreen` (wired to `MobileDayLog`'s FAB). **Food tab:** own search + horizontally-scrolling category pills + result list with complement-score badges (`computeComplementScore`, sorted descending when a DV profile is active, A–Z fallback otherwise), gram-confirm step reusing `MobileGramInput`, diet-suggestions row at top (140×110px cards, top-2 gap tags, `+ Log to Today`; 3 empty states — hidden with no profile, "Set up your diet on desktop…" with no diet list, congratulations copy when all gaps are filled). **Meal tab:** My Templates (`loadSavedMeals()`) + Presets (`loadPresetMeals()`), same score badges. **Plan tab:** saved plans (`loadMealPlans()`). All three logging paths write the exact `addEntry()` payload shapes desktop's `CalendarAddModal.tsx` uses (`entry_type` + `meal_label` handling identical). `onEntriesChanged` triggers a re-fetch of the currently-loaded date range in `MobileCalendarScreen`. Verified against live Supabase data: `tsc --noEmit` and `next build` clean. |
| **Mobile — Phase 7: Polish + Safe Areas** | ✅ Complete — final phase of the mobile build. New shared `components/mobile/useSwipeToDismiss.ts` hook wired into all 3 bottom sheets (`MobileDVProfileSheet`, `MobileNutrientInfoSheet`, `MobileAddSheet`) for drag-to-dismiss; `PortraitOverlay` added to `MobileShell`; safe-area-inset-bottom padding added to the sheets that were missing it; scale-dip touch feedback added to the FAB, both primary "Log" CTAs, and all food/meal/plan row buttons; `MobileFoodSearch`'s temporary category `<select>` replaced with a pill row (zero `<select>` elements remain in `components/mobile/`); week-strip nav arrows narrowed (`w-11`→`w-8`) to give the 7 day columns more room, with a documented trade-off that column *width* can't reach 44px at typical phone widths while 7 columns + nav arrows share ~320px (height does meet 44px). `tsc --noEmit` clean; `next build` clean; dev-server smoke test of both `/` and `/m` against live Supabase data, no runtime errors. **No browser/device tool was available this session** (Chrome extension declined) — viewport-dependent items (no-horizontal-scroll, FAB clearance, keyboard-covering-a-sheet) were verified by code audit (relative units, no fixed pixel widths outside their own scroll containers) rather than visually; give the app a real-device pass before shipping. Full per-checklist-item detail in `MOBILE_BUILD_PLAN.md` → Phase 7. **This completes all 11 phases (0–7) of the mobile build.** |
...
- [x] Mobile-responsive collapse — superseded by the `/m` mobile build; all 11 phases complete (2026-08-12). Remaining: real-device visual QA pass (no browser tool available during the build).
...
**All 11 phases (0–7) complete (2026-08-12).** The mobile build (`/m`) is feature-complete: Calendar (week strip, day log, macro chips, streak, donut/radar visualization), Nutrition (search, gram/unit controls, full nutrient accordion, info sheets), Account (auth), DV Profile selection, and the Add Food/Meal/Plan sheet, all polished per Phase 7's app-like-behavior checklist. Verified against live Supabase data throughout (this session's `.env.local` is local-only, gitignored — not committed). **Not yet done:** a real mobile-device/browser visual pass (no browser tool was available this session) — recommended next step before considering `/m` ship-ready. See `MOBILE_BUILD_PLAN.md` → "Verified API Reference" and "Revision Notes" for corrections made against the live codebase on 2026-08-07.

**Phase 1 summary:** `/m` now renders a blank slate-900 shell with a top header (title + DV profile chip placeholder) and a bottom tab bar (Calendar / Nutrition / Account, inline-SVG icons, safe-area-aware). `MobileShell` owns tab state and locks body scroll while mounted (restored on unmount, since `/` and `/m` share one root `<body>`). Screens themselves are still placeholders — Ph-2 through Ph-5 fill them in. **Ph-1 payload check (resolved 2026-08-12):** `.env.local` now configured (Supabase project `ieqrdzffpotiedipffka`); `next build`'s static `/m` output is 189 KB raw RSC / ~51 KB gzipped — well under the ~500 KB budget flagged in the Ph-1 write-up. No follow-up needed.

**Phase 0 summary:** `app/layout.tsx` previously rendered a `md:hidden` "Open on Desktop" gate and wrapped children in `hidden md:contents`, which would have kept anything under `app/m/` at `display: none` on every mobile viewport. Phase 0 moved the desktop page and gate into a new `app/(desktop)/` route group (`app/(desktop)/layout.tsx` + `app/(desktop)/page.tsx`, route group adds no URL segment so `/` is unaffected), leaving the root `app/layout.tsx` as just html/body/fonts/metadata/`AuthProvider`. Old `app/page.tsx` deleted (content moved verbatim). A violet "Open the mobile app →" link (`href="/m"`) was added to the gate copy per Option A. `app/m/` does not exist yet — it is created in Phase 1, along with the `viewport`/`metadata` exports (`viewportFit: 'cover'` etc.) specified in the Phase 0 write-up. Verified with `tsc --noEmit` (clean) and `next build` (compiles and resolves routes correctly; fails only at the page-data-collection step due to missing local `.env.local` Supabase credentials — a pre-existing environment gap, not a Phase 0 regression). `npm install` was run in this environment since `node_modules/` was absent.
...
user_rda_profiles   (per user)    — Also stores a sentinel row named `__np_prefs__` (filtered from the UI list) whose `values` JSONB holds account preferences: `{ default_profile: string }`. No separate table needed.
