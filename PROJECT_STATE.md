...
| **Default DV Profile** | ✅ Live — star (☆/★) toggle on every profile row in the DV Profile overlay picker (None, four built-ins, each saved profile); logged-in only; filled amber ★ = current account default; clicking a star saves to `user_preferences` Supabase table via upsert; clicking the active star clears it (deletes the row); on login the DB default overrides localStorage and is applied immediately; no default set = localStorage behaviour unchanged; `lib/userPreferencesStorage.ts` created (load / save / delete); `AppShell` loads prefs + saved profiles in parallel on login via `Promise.all`; new props `defaultProfileKey` + `onSetDefault` wired from `AppShell` → `DVProfilePanel`; built-in profile rows restructured from plain `<button>` to `<div>`+button+star in overlay picker |
...
user_preferences    (per user)    — Account-level settings; currently stores `default_profile TEXT` (rdaSelection string: '', 'male-avg', 'female-avg', 'male-lowcarb', 'female-lowcarb', 'saved:<uuid>'); one row per user; upsert on save; row deleted when default is cleared; RLS owner-only
